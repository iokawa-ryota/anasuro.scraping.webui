...
- `apps/setup.bat` を改善し、Python未導入時は `winget` で自動インストールを試行するように変更
- `python` と `py -3` の両方を検出対象に追加
- インストール直後に認識できない場合の再実行案内を追加

### ドキュメント

- `apps/README.md` に自動インストール前提（インターネット接続/winget）を追記
- `apps/かんたんガイド.txt` に同内容を反映
